...
# function that generate the list of actions
def generate_actions(node_list):
    ...
    for c in node_list:
        cr = CR_Action(0, id, state, c, "change reference copy",
                       "change reference copy, to a copy hosted in node " + str(c.id), c.mean_delay,
                       np.loadtxt("ICR" + str(c.id) + ".txt", delimiter=","), "ICR" + str(c.id) + ".txt", cost_cr,
                       "ICR" + str(c.id))
        id = id + 1
