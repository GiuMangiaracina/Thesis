...
def main():
    ...
    # execution time [s]
    r3 = requests.get(
        "http://127.0.0.1:1808" + str(actions.c_id) + "/api/v1/applications/" + str(id) + "/executors").json()
    t = r3[0]
    ...
    r3 = requests.get(
        "http://127.0.0.1:1808" + str(actions.c_id) + "/api/v1/applications/" + str(id) + "/stages/1").json()
    re = r3[0]
    rec = re["inputRecords"]
